...
class War():
    def __init__(self):
        ...
        ''' Stack because it is distributed from the top to the self.PayingPile and other.PlayingPile  '''
        self.myPayingPile = Stack()	#Nice Spelling
        ''' The cards are "flipped" from the top of the stack '''
        ####
        ''' Saved into a queue because you let it build up but then in real life you flip your cards over which would indicate that the first one in is the first one out '''
        ...
        ''' The cards are "flipped" from the top of the stack  '''
        self.lootPile = Queue()
        ''' Stored into a queue so that when we add it to the piles it can be done in a first in first out type of manner '''
